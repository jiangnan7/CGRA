...
class Arch(Base): 
    ...
    def _parseSwitchs(self): 
        ...
        self._switches = {}
        for name in info.keys(): 
            self._switches[name] = Switch(name, info[name]) # self._switches["FULLYCONN_1X3"] = Switch("FULLYCONN_1X3", {"input": [], "output": [], ……})

    def _parseModules(self): 
        info = self._modulesRaw
        self._modules = {}
        for name in info.keys(): 
            self._modules[name] = Module(name, info[name]) # self._modules["BlockARI"] = Switch("BlockARI", {"input": [], "output": [], ……})
        for name in info.keys(): 
