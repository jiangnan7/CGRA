...
class Switch(Base): 
    def __init__(self, name, info = {}): 
        assert("FULLY" in name or "ANY" in name)
        if("FULLY" in name):
            if len(info) == 0: 
        ...
        if("ANY" in name):
            self._name           = name
            self._firstinputs    = info["first"]["input"].copy()
            self._firstoutputs   = info["first"]["output"].copy()
            self._firstrequired  = info["first"]["required"].copy()
            self._firstgraph     = info["first"]["graph"]
            self._secondinputs   = info["second"]["input"].copy()
            self._secondoutputs  = info["second"]["output"].copy()
            self._secondrequired = info["second"]["required"].copy()
            self._secondgraph    = info["second"]["graph"]
            self._thirdinputs    = info["third"]["input"].copy()
            self._thirdoutputs   = info["third"]["output"].copy()
            self._thirdrequired  = info["third"]["required"].copy()
            self._thirdgraph     = info["third"]["graph"]
            self._fourthinputs   = info["fourth"]["input"].copy()
            self._fourthoutputs  = info["fourth"]["output"].copy()
            self._fourthrequired = info["fourth"]["required"].copy()
            self._fourthgraph    = info["fourth"]["graph"]
            self._inputs    = []
            self._outputs   = []
            self._required  = []
            self._graphAbs  = HyperGraph()
            self._graphInfo = []
            self._graph     = HyperGraph()
            for name in self._firstinputs:
                self._graphAbs.addVertex(name, {"type": "__MODULE_INPUT_PORT__", "device": self._name + "." + name, "switch": self._name})
            for name in self._firstoutputs:
                self._graphAbs.addVertex(name, {"type": "__ELEMENT_INPUT_PORT__", "device": self._name + "." + name, "switch": self._name})
            for name in self._secondinputs:
                self._graphAbs.addVertex(name, {"type": "__ELEMENT_OUTPUT_PORT__", "device": self._name + "." + name, "switch": self._name})
            for name in self._secondoutputs:
                self._graphAbs.addVertex(name, {"type": "__ELEMENT_INPUT_PORT__", "device": self._name + "." + name, "switch": self._name})
            for name in self._thirdinputs:
                self._graphAbs.addVertex(name, {"type": "__ELEMENT_OUTPUT_PORT__", "device": self._name + "." + name, "switch": self._name})
            for name in self._thirdoutputs:
                self._graphAbs.addVertex(name, {"type": "__MODULE_OUTPUT_PORT__", "device": self._name + "." + name, "switch": self._name})
            for name in self._fourthinputs:
                if self._graphAbs.vertex(name) == None:
                    self._graphAbs.addVertex(name, {"type": "__ELEMENT_INPUT_PORT__", "device": self._name + "." + name, "switch": self._name})
            for name in self._fourthoutputs:
                if self._graphAbs.vertex(name) == None:
                    self._graphAbs.addVertex(name, {"type": "__ELEMENT_OUTPUT_PORT__", "device": self._name + "." + name, "switch": self._name})
            # for name in self._fourthinputs:
            #     assert self._graphAbs.vertex(name) != None
            # for name in self._fourthoutputs:
            #     # print(name)
            #     assert self._graphAbs.vertex(name) != None

            for conn in self._firstrequired:
                splited = conn.split("->")
                assert len(splited) == 2
                fr, to = splited
                assert fr in self._graphAbs.vertices(), "Switch: Invalid edge source: " + fr
                assert to in self._graphAbs.vertices(), "Switch: Invalid edge sink: " + to
                self._graphAbs.addNet([fr, to], {})
            for conn in self._secondrequired:
                splited = conn.split("->")
                assert len(splited) == 2
                fr, to = splited
                assert fr in self._graphAbs.vertices(), "Switch: Invalid edge source: " + fr
                assert to in self._graphAbs.vertices(), "Switch: Invalid edge sink: " + to
                self._graphAbs.addNet([fr, to], {})
            for conn in self._thirdrequired:
                splited = conn.split("->")
                assert len(splited) == 2
                fr, to = splited
                assert fr in self._graphAbs.vertices(), "Switch: Invalid edge source: " + fr
                assert to in self._graphAbs.vertices(), "Switch: Invalid edge sink: " + to
                self._graphAbs.addNet([fr, to], {})
            for conn in self._fourthrequired:
                splited = conn.split("->")
                assert len(splited) == 2
                fr, to = splited
                assert fr in self._graphAbs.vertices(), "Switch: Invalid edge source: " + fr
