...
bool HyperGraph::dumptoRRG(const std::string &path) const
{
    ofstream fout(path);
    if (!fout) {
        return false;
    ...
    for (const auto &vertex : _vertices) {
        fout << "vertex " << vertex.first << endl;
        for (const auto &attr : vertex.second.attrs()) {
            fout << "attr " << attr.first;
            if (attr.second.isStr()) {
                fout << " str " << attr.second.getStr();
            } else if (attr.second.isInt()) {
                fout << " int " << attr.second.getInt();
            }
            fout << endl;
        }
    }
    for (const auto &edges : _netsOut) {
        for (const auto &edge : edges.second) {
            fout << "net " << edge.from() << " " << edge.to() << endl;
...
void HyperGraph::deleteFu(const std::string &fuName) 
{
    vector<string> vertexToDelete;
    for (const auto &_ : _vertices) { 
        if ((_.first.find(fuName + ".in") != _.first.npos) || (_.first.find(fuName + ".out") != _.first.npos) || (_.first == fuName)) { 
            vertexToDelete.push_back(_.first);
            clog << "vertex to be deleted: " << _.first << endl;
        }
    }

    for (const auto &_ : vertexToDelete) {
        delVertex(_);
    }
}

void HyperGraph::deleteFu(std::string &&fuName)
{
    vector<string> vertexToDelete;
    for (const auto &_ : _vertices) {
        if (_.first.find(fuName) != _.first.npos) {
            vertexToDelete.push_back(_.first);
            clog << "vertex to be deleted: " << _.first << endl;
        }
    }
