...
std::unordered_map<std::string, pair<int,int>> readPos(const std::string &filename){
    ...
    while(!fin.eof())
    {
        ...
        int temp1;
        int temp2;
        result[tmp] = pair<int,int>(); 
        while(!sin.eof())
        {
            string tmp2;
            sin >> tmp2; 
            temp1 = str2num<int>(tmp2);
            sin >> tmp2; 
            temp2 = str2num<int>(tmp2);
            if(tmp2.empty())
            {
                continue; 
            }
            result[tmp] = pair<int,int>(temp1, temp2); 
        }
    }
