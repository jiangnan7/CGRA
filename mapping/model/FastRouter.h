...
public:
    ...
    bool pathUsed(const std::pair<std::string, std::string> &edge, const std::string &signal) const { return(_paths.find(edge.first) != _paths.end() && _paths.find(edge.first)->second.find(edge.second) != _paths.find(edge.first)->second.end()) && (_paths.find(edge.first)->second.find(edge.second)->second.empty() || _usedSignal.find(*_paths.find(edge.first)->second.find(edge.second)->second.begin())->second == signal); } 
    bool pathUsed_cache(const std::pair<std::string, std::string> &edge, const std::string &signal) const { return _cache.find(edge.first + "->" + edge.second) != _cache.end(); }
    ...
    bool norm_route(const std::vector<std::pair<std::string, std::string>> &edgesToMap, const std::vector<std::string> &edgesSignal, 
                const std::unordered_map<std::string, std::unordered_map<std::string, std::vector<std::string>>> &pathsGiven = {});
    std::unordered_map<std::string, std::string>       &hardPath()       {return hardPaths; }            
    std::unordered_map<std::string, std::string> hardPaths; 
